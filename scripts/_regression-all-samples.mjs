/* 9 샘플 packBestWithCandidateUnion 시뮬 — 사용자 수정 후 데이터 기준 */
import fs from "node:fs";
const { packBestWithCandidateUnion } = await import("../lib/packing/algorithm.ts");
...
console.log("=== 9 샘플 packBestWithCandidateUnion 결과 (DB 최신 데이터) ===");
console.log("id | name | 컨 셋 | 미배치 | pack 시간 | 후보 시도");
console.log("---");
for (const s of SAMPLES) {
  if (!fs.existsSync(s.file)) {
    console.log(`${s.id} | ${s.name} | SKIP (파일 없음)`);
    continue;
  }
  ...
  const candTried = r.debug?.candidatesEvaluated?.length ?? 0;
  const unplaced = r.unplaced.reduce((s, u) => s + (u.quantity ?? 1), 0);
  console.log(
    `${s.id.padEnd(11)} | ${s.name.padEnd(13)} | ${set.padEnd(15)} | ${unplaced.toString().padStart(3)} | ${(dt / 1000).toFixed(1).padStart(6)}초 | ${candTried}`,
  );
}
console.log("---");
