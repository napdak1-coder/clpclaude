/* sg-1-14 — residualMakeRoom 시도 (Codex 구현 활용).
 * runner config 만, algorithm.ts 수정 없음.
 */
...
function analyze(result) {
  ...
  return { unplCids, unplUnits, cargoSplit: [...cargoCi.values()].filter((s) => s.size > 1).length, bookingSplit: [...bkCi.values()].filter((s) => s.size > 1).length, auditPass: audit.pass && audit.violations.length === 0, softCbm, hardCbm, weightOver, set: result.containers.map((c) => c.spec.type).join("+") };
}
...
console.log("# sg-1-14 residualMakeRoom 시도");
console.log(`date: ${new Date().toISOString()}`);
console.log("| config | unpl | cargoIds | cargoSplit | bookingSplit | audit | hardCbm | wtOver | pack(s) |");
console.log("|---|---:|---|---:|---:|---|---:|---:|---:|");

const CONFIGS = [
  { name: "base (3/12/50/60s)", opts: { maxRemoveCargoIds: 3, maxRemoveUnits: 12, maxTargetsPerCargo: 50, timeBudgetMs: 60_000 } },
  { name: "agg-A (4/16/100/120s)", opts: { maxRemoveCargoIds: 4, maxRemoveUnits: 16, maxTargetsPerCargo: 100, timeBudgetMs: 120_000 } },
];
...
for (const cfg of CONFIGS) {
  ...
  const a = analyze(r);
  console.log(`| ${cfg.name} | ${a.unplUnits} | ${a.unplCids.join(",") || "-"} | ${a.cargoSplit} | ${a.bookingSplit} | ${a.auditPass ? "P" : "F"} | ${a.hardCbm} | ${a.weightOver} | ${dt.toFixed(1)} |`);
}
