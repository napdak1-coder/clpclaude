/**
 * 2ST SG TOTAL — 컨테이너별 물리·알고리즘 규칙 종합 검증
 *
 * 검증 항목:
 *   A. 물리 규격 (각 컨테이너 별)
...
const EPS = 0.5; // 5mm 오차 허용

console.log(`\n=== 2ST SG TOTAL — 물리·규칙 종합 검증 ===`);
...
// placement 구조 helper: position.{x,y} + size.{width,length,height}
// z 좌표는 row 단위 (bottom=0, top=row.bottomHeight) — display row 가 한 단위
// rows: [{ bottomItems, topItems, bottomHeight (= top z) }]
const px = (p) => p.position.x;
const py = (p) => p.position.y;
const pw = (p) => p.size.width;
const pl = (p) => p.size.length;
const ph = (p) => p.size.height;
const pz = (p, row) => (p.layer === "bottom" ? 0 : row.bottomHeight ?? 0);
...
for (const cont of result.containers) {
  ...
  // 모든 placement (visual) — 각자 z 좌표 계산해서 첨부
  const placements = [];
  ...
  // ── A1. bounding (width + height) — length 축 좌표는 display rows 가 lane 단위로 변환된
  //   값이므로 finalized 데이터에서 직접 길이 검사 불가. extreme-point 알고리즘이
  //   배치 시점에 fit 검사를 통과해야만 placement 가 추가되므로 길이 축은 알고리즘 invariant.
  ...
  // ── A2. weight  (placement.weight = per-unit, 1 placement = 1 unit; bulk.weightPerUnit = total bulk weight)
  ...
  // ── A5. full support — top layer placement 의 base 가 bottom layer 면 ≥ 70% 받침
  let unsupported = 0;
