/**
 * 빠른 fallback 검증 — 3ST SG TOTAL 을 packBest({ lightMode: true }) 로 호출.
 * 매트릭스 56 → 12. 시간 < 2분 목표, 미배치 0 도달 Y/N.
 */
...
console.log(`JSON 샘플: ${rows.length} 행 — packBest({lightMode:true}) 호출`);

const t0 = Date.now();
const result = packBest(cargoes, "auto", { lightMode: true });
