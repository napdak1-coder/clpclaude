/**
 * 1ST SG TOTAL — 실무자 분배가 물리적으로 유효한지 검증.
 * pack() 에 fixedContainers (40FT, 20FT) + fixedAssignment (실무자 명시 매핑) 강제 후
 * 컨테이너별 A1~A5 / B1 / B3 검증.
 */
...
const sample = JSON.parse(
  fs.readFileSync(path.resolve("data/samples/singapore-total.json"), "utf8"),
);

const PRACTITIONER = {
  1: ["메가젠임플란트", "데코론", "YKMC", "보현석재", "에이제이테크", "카페봄봄", "EXCELERATE ENERGY", "VISCOSMO", "더블유티 스프레이", "리만", "대한정밀공업", "선진뷰티사이언스", "SUNGBO INDUSTRIA", "제일기공", "웨스코", "디에스콘", "티케이테크"],
  2: ["AWOT", "대원산업", "씨에스에프", "HD현대건설기계", "포컴퍼니"],
};
...
// 화주 → cargoId (같은 화주에 여러 행이면 모두 매핑)
const shipperToIds = new Map();
for (const c of cargoes) {
  const list = shipperToIds.get(c.actualShipperName) ?? [];
  list.push(c.id);
  shipperToIds.set(c.actualShipperName, list);
}

const fixedAssignment = {};
for (const [contIdx, shippers] of Object.entries(PRACTITIONER)) {
  for (const sh of shippers) {
    const ids = shipperToIds.get(sh);
    if (!ids || ids.length === 0) {
      console.error(`매칭 실패: 실화주 "${sh}" 가 sample 에 없음`);
      process.exit(2);
    }
    for (const id of ids) fixedAssignment[id] = Number(contIdx);
  }
}

const fixedContainers = ["40FT", "20FT"];

console.log("=== 1ST SG TOTAL — 실무자 분배 물리 검증 ===");
console.log(`fixedContainers: ${fixedContainers.join(", ")}`);
console.log(`fixedAssignment: ${cargoes.length} cargo → 컨1=${PRACTITIONER[1].length} 화주, 컨2=${PRACTITIONER[2].length} 화주`);
...
if (result.unplaced.length > 0) {
  ...
  for (const u of result.unplaced) {
    ...
    console.log(`   - ${cg?.actualShipperName ?? "?"} (id=${cg?.id ?? "?"}, qty=${u.quantity ?? 1}, size=${cg?.width}×${cg?.length}×${cg?.height}, cbm=${cg?.aboutCbm ?? cg?.cbm ?? "?"}, weight=${(cg?.weightPerUnit ?? 0) * (cg?.quantity ?? 1)}kg)`);
  }
  console.log("");
}
