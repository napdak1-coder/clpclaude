...
## 4.6단계: 긴 막대형 박스 모서리 박음 (장축 고정점 — 차선책 모드)
...
- 그러나 무조건 막대형을 먼저 박으면 다른 시나리오(3ST SG VPHI 박스 묶음)에서 회귀 발생

**한 줄 원리**

> "평소엔 꺼져 있다. 미배치가 1개라도 생기면 막대형을 컨테이너 안쪽 모서리부터 차례로 박은 뒤 다시 시도."

### 동작 방식 — 차선책 모드 (default off)

| 단계 | 동작 |
|---|---|
| 일반 시도 | `longAxisAnchor` 비활성 (기본값) — 발바닥 사전 묶음만 가동 |
| 미배치 감지 | `packBest` 매트릭스(56가지) 끝나도 미배치 > 0 인 경우 |
| 차선책 발동 | `longAxisAnchor: { enabled: true }` 로 매트릭스 한 바퀴 더 |
| 결과 비교 | lex 비교 ① 미배치 적은 것 → ② 마감 → ③ 충전률 → ④ 균형 |
| 채택 | 차선책이 더 좋으면 갱신, 아니면 원래 결과 유지 |

### 활성 시 막대형 박음 절차 (`anchorLongAxisCargoes`)

| 항목 | 내용 |
|---|---|
| 발동 임계 | 화물 한 행의 최대 변 ≥ 300 cm |
...
| 시나리오 | 일반 매트릭스 | 차선책 발동 후 |
|---|---|---|
| 1ST/2ST SG, 2ST HM | 미배치 0 → 차선책 발동 X (회귀 0건) | 동일 |
| 3ST SG (311 cm 롱빔 포함) | 미배치 1건 (sg3-8 세아특수강) | 차선책 발동 → 0건 도달 시도 |

→ 일반 시나리오 영향 없음, 막대형 회귀 시나리오에서만 추가 시도.
