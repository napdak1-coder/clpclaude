...
- 발바닥 사전 묶음에 룰 H를 추가했다. 단독 부킹, 같은 화물, 2~3개 큰 정사각형 계열 박스만 바닥에 먼저 나란히 깐다.
- 긴 막대형 박스가 잘못 잡히지 않도록 가로세로 비율 0.9 이상, 바닥 면적 15,000㎠ 이상일 때만 발동한다. 이 조건 때문에 `sg3-13` 같은 길쭉한 화물은 제외된다.
- 잔여 자리 만들기의 묶음 목표가 서로 다른 크기의 박스도 한 묶음으로 볼 수 있게 보강됐다. `sg-5-35`의 실제 데이터는 166×166×83 1개와 136×136×72 1개라서 이 보강이 필요했다.
- 결과: 화면 기준 엄격 진단 10개 중 8개가 미배치 0으로 올라갔다. 새로 해결된 건 `sg-5-35`, 남은 건 `mangjak-10`, `sg-1-14` 두 건이다.
- 기본 운영 경로 9개 샘플은 모두 미배치 0을 유지했다. 단위 테스트는 93개 통과.

## 2026-05-14 추가 진행 기록 2

- 남은 두 건은 같은 원인이 아니었다. `mangjak-10`은 컨테이너 후보 선택 문제, `sg-1-14`는 긴 화물이 작은 후보 셋에서 직선 바닥 자리를 못 얻는 문제로 분리했다.
- `packBestWithCandidateUnion`의 후보 생성에 "부피상 최소 후보"뿐 아니라 같은 대수의 더 넓은 후보와 한 단계 큰 후보를 넣었다. 후보는 반드시 실제 적재 후 미배치 0, 화물 분산 0, 부킹 분산 0, 부피/중량 초과 0, 감사 통과일 때만 채택된다.
- 빈 컨테이너를 뺀 대안도 후보로 다시 검증한다. 화면에만 보이는 대안이 아니라 실제 물리 검사를 통과해야 결과가 된다.
- 집중 확인: `mangjak`은 엄격 화면 기준 + 잔여 자리 만들기에서 미배치 0, `sg-1`도 같은 조건에서 미배치 0. `sg-1`은 후보 확장 경로에서 40FT+40FT가 미배치 0으로 확인됐다.
- 기본 운영 경로 9개 샘플은 미배치 0 유지. 단위 테스트는 95개 통과.
- 전체 10개를 한 번에 돌리는 검증은 시간이 매우 커서 중단했다. 이후 전체 검증은 샘플별 시간 제한을 걸어 나눠서 실행한다.

## 2026-05-14 추가 진행 기록 3

- 외부 PackingSolver 도입은 보류하고, 내부 휴리스틱을 병렬 탐색으로 보강하는 방향으로 정했다.
- 새 opt-in 경로 `packBestWithAdaptiveParallelCandidateUnion`을 추가했다. 기본 `packBest`와 기존 `packBestWithCandidateUnion`은 그대로 유지한다.
- 병렬 경로는 컨테이너 후보 × 정렬 전략 × 컨테이너 시도 순서 × 배치 모드 × 자동마감 × 장축 앵커 조합을 독립 시도로 만들고, 컴퓨터 코어 수/메모리/샘플 크기에 맞춰 동시 실행 수를 자동 산정한다.
- 병렬 탐색 전용 안전 정렬 후보 3개를 추가했다. `long-cargo-first`는 긴 화물의 직선 바닥 자리를 먼저 확보하고, `largest-footprint-first`는 큰 발바닥 화물을 먼저 깔며, `no-stacking-first`는 다단금지 화물의 바닥 자리를 먼저 확보한다. 세 후보 모두 규칙 완화 없이 순서만 바꾼다.
- 결과 선택은 기존과 같은 lex 기준이다. 미배치, 감사 실패, 부피/중량 초과, 화물 분산, 부킹 분산, 컨테이너 수, 총 용량 순으로 비교하며 점수 합산은 쓰지 않는다.
- `/api/pack`에는 `useParallelCandidateUnion: true` 옵션만 추가했다. 기본 호출은 기존 동작을 유지한다.
- 검증: 병렬 모듈 import 성공, worker 실행 smoke test 성공, 병렬 테스트 포함 단위 테스트 100개 통과. 기본 운영 경로 9개 샘플은 모두 미배치 0 유지. 전체 `tsc --noEmit`은 기존 타입 오류들이 남아 있어 별도 정리 필요.
- 첫 적용 결과: `sg-1` strict visual 단일 booking-cluster 기준은 `sg-1-14` 1개 미배치였지만 병렬 탐색은 `40FT+20FT` 안에서 미배치 0을 찾았다(44.2초). `mangjak` strict visual 단일 ldf 기준은 `mangjak-10` 1개 미배치였지만 병렬 탐색은 `40FT+20FT` 안에서 미배치 0을 찾았다(7.1초). 두 결과 모두 화물 분산 0, 부킹 분산 0, 감사 통과.
- 안전 정렬 후보 확장 후 재검증: `sg-1` 병렬 탐색은 46.8초, `mangjak` 병렬 탐색은 9.0초에 각각 미배치 0, 화물 분산 0, 부킹 분산 0, 감사 통과를 유지했다. 기본 운영 9개 샘플도 모두 미배치 0 유지.
- 제한 없는 전체 strict visual 검증에서 큰 싱가포르 샘플이 오래 붙잡히는 병목을 확인했다. 원인은 이미 미배치 0, 감사 통과, 분산 0 결과를 찾은 뒤에도 같은 컨테이너 후보 순위의 남은 배치 순서를 계속 실행하는 것이었다.
- 병렬 실행기는 가장 작은 후보 순위에서 valid 결과를 찾으면 그 순위 이상의 아직 시작하지 않은 시도는 건너뛴다. 이미 실행 중인 worker는 자연 종료를 기다리고, 결과 선택 기준은 기존 lex 비교 그대로라 점수 합산이나 규칙 완화는 없다.
- 전체 검증 스크립트는 샘플 단위도 child process 로 분리했다. 샘플 2개가 동시에 START 되고, 각 샘플 안에서 병렬 후보 worker 를 다시 펼친다.
- `skipInitialProbe` 옵션을 추가했다. 이 옵션은 사전 단일 배치 결과를 기다리지 않고 declared/physical CBM 에서 컨테이너 후보를 바로 만든 뒤 병렬 후보를 시작한다. `/api/pack`에서는 `parallelSkipInitialProbe: true`로만 켜진다.
